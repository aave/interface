...
interface SupplyWrappedTokenActionPropsSDK extends BoxProps {
  tokenIn: string;
...
export const SupplyWrappedTokenActionsSDK = ({
  tokenIn,
  amountToSupply,
  decimals,
  symbol,
  ...
  reserve,
  ...propsx
}: SupplyWrappedTokenActionPropsSDK) => {
  ...
  const action = async () => {
    try {
      ...
      addTransaction(response.hash, {
        ...
        amount: amountToSupply,
        assetName: symbol,
        amountUsd: valueToBigNumber(amountToSupply)
          .multipliedBy(reserve.usdExchangeRate)
          .toString(),
      });
