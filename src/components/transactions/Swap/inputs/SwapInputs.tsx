...
export const SwapInputs = ({
  ...
  const handleInputChange = (value: string) => {
    ...
    if (value === '-1') {
      // Max Selected
      setState({ inputAmount: state.sourceToken.balance });
      setState({ debouncedInputAmount: state.sourceToken.balance });
      setState({ isMaxSelected: true });
    } else {
      setState({ inputAmount: value });
      setState({ debouncedInputAmount: value });
      setState({ isMaxSelected: value === state.forcedMaxValue });
    }
  ...
  const onSwitchReserves = () => {
    ...
    const toInput = state.swapRate
      ? normalizeBN(state.swapRate.destAmount, state.swapRate.destDecimals).toString()
      : '0';

    setState({ sourceToken: toToken });
    setState({ destinationToken: fromToken });
    setState({ inputAmount: toInput });
    setState({ debouncedInputAmount: toInput });
