...
export const HelpModalSupplyContent = () => {
  const { pagination, totalPagination, tourInProgress, setPagination } = useHelpContext();
  const { close } = useModalContext();

  const { breakpoints } = useTheme();
  const xsm = useMediaQuery(breakpoints.down('xsm'));

  let title, description, top, right, pagesInTour;
  const page = pagination[tourInProgress as keyof typeof pagination];
