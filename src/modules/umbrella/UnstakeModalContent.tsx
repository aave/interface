...
export const UnStakeModalContent = ({ stakeData }: { stakeData: MergedStakeData }) => {
  ...
  const { data: redeemedAmount, isLoading } = usePreviewRedeem(
    ...
    stakeData.decimals,
    stakeData.stakeTokenUnderlying,
    currentChainId
  );
  ...
  const redeemedAmountUsd = new BigNumber(redeemedAmount || '0')
    .multipliedBy(stakeData.stakeTokenPrice)
    .shiftedBy(-8);
  ...
  const symbolFormatted = stakeData.underlyingIsWaToken
    ? stakeData.waTokenData.waTokenUnderlyingSymbol
    : stakeData.underlyingTokenSymbol;

  const isWrongNetwork = currentChainId !== connectedChainId;

  let hfAfterRedeem = '-1';
  if (user && stakeData.underlyingIsWaToken && redeemATokens) {
    const poolReserve = reserves.find(
      (r) =>
        r.underlyingAsset.toLowerCase() === stakeData.waTokenData.waTokenUnderlying.toLowerCase()
    );

    if (!poolReserve) {
      throw new Error('Pool reserve not found for underlying asset');
    }
  ...
  return (
    <>
      <TxModalTitle title="Unstake" symbol={symbolFormatted} />
